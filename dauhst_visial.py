...
if __name__ == '__main__':
    modle = DAUHST(num_iterations=3).cuda()
    Phi = torch.rand([5, 28, 256, 310]).cuda()
    Phi_s = torch.rand([5, 256, 310]).cuda()
    input_mask = [Phi, Phi_s]
    y = torch.rand([5, 256, 310]).cuda()
    out_put = modle(y, input_mask)