...
result_path = 'ablation_test2'+ '/' + opt.save_name + '/result/'  # train_t_mask 为1_1
model_path = 'ablation_test2' + '/' + opt.save_name + '/model/'
...
spec_model = unet(CHANNEL=21, channel_nums=64, kernel_size=5, raw_flag=False).cuda()
spec_model.load_state_dict(torch.load('ablation_test/cave_data_cnn_only_spec/model/model_epoch_197.pth'))
for param in spec_model.parameters():
    param.requires_grad = False



model = unet_polar(CHANNEL=21, channel_nums=21*3*2, kernel_size=5, raw_flag=False).cuda()
...
# filter_matrix = filter_matrix.mean(dim=1, keepdim=True)
# filter_matrix_test = filter_matrix_test.mean(dim=1, keepdim=True)
...
def train(epoch, logger):
    ...
    for i in tqdm(range(batch_num), desc=f'Epoch {epoch}', unit='batch'):
        ...
        gt_batch = shuffle_crop_npy_spec_polo(train_set, opt.batch_size, opt.crop_size)  # 9 21 4 200 200
        spec_data = gt_batch.clone()

        spec_data = spec_data[:, :, 0, :, :].unsqueeze(2)  # 9 21 1 200 200
        spec_mask = noisy_filter[:, :, 0, :, :].unsqueeze(2)  # 9 1 1 200 200
        with torch.no_grad():
            spec_out = spec_model(spec_data, spec_mask)  # 9 21 1 200 200
 
        # print(f'spec_out.shape: {spec_out.shape}')  
        ...
        model_out = model(spec_out, gt_batch, noisy_filter)  # 9 16 200 200
        model_out = model_out.view(opt.batch_size, 21, 3, opt.crop_size, opt.crop_size)
        # print(f'model_out.shape: {model_out.shape}')  # 5 21 4 100 100
        ...
        pre_loss_s123 = L1_loss(out_s123, gt_s123)
        loss = pre_loss_s123
    ...
    model_out = torch.concat([spec_out.unsqueeze(2), model_out], dim=2)  # 9 21 4 200 200
    for k in range(model_out.shape[0]):

        psnr_train= torch_psnr(model_out[k, :, 0, :, :], gt_batch[k, :, 0, :, :])
        ssim_train = torch_ssim(model_out[k, :, 0, :, :], gt_batch[k, :, 0, :, :])
...
def test(epoch, logger):
    ...
    with torch.no_grad():
        for k in range(N):
            ...
            spec_data = gt_tensor[:, :, 0, :, :].unsqueeze(2)  # 9 21 1 200 200
            spec_mask = noisy_filter_test[:, :, 0, :, :].unsqueeze(2)  # 9 1 1 200 200
            with torch.no_grad():
                spec_out = spec_model(spec_data, spec_mask)  # 9 21 1 200 200

            out_tensor = model(spec_out, gt_tensor, noisy_filter_test)
            out_tensor = out_tensor.view(1, 21, 3, H, W)
            out_tensor = torch.concat([spec_out.unsqueeze(2), out_tensor], dim=2)  # 9 21 4 H W
...
def main():
    ...
    for epoch in range(1, opt.max_epoch + 1):
        ...
        # if (psnr_mean > psnr_max and psnr_mean != np.inf):
            # psnr_max = psnr_mean 
        name = result_path + '/' + 'Test_%d_%.2f_%.3f_%.4f_%.4f' % (epoch, psnr_mean, ssim_mean, dop_err_mean, cpf_err_mean)
