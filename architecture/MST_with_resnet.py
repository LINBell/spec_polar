...
class resnet(nn.Module):
    def __init__(self,CHANNEL, channel_nums=64, kernel_size=5, layers_num=10):
...
class MST_with_resnet(nn.Module):
    def __init__(self, dim=21, stage=3, num_blocks=[2,2,2], need_noise=False, num_sp=5, need_mask_atten=False, raw_flag=False, resnet_layers=10):
        super(MST_with_resnet, self).__init__()
        self.dim = dim
        self.stage = stage
        self.need_noise = need_noise
        self.num_sp = num_sp
        ...
        # self.resnet = resnet(CHANNEL=self.dim, channel_nums=64*2, kernel_size=5, layers_num=resnet_layers)
        self.resnet = resnet(CHANNEL=self.dim, channel_nums=64, kernel_size=5, layers_num=resnet_layers)
        # Encoder
    ...
    def forward(self, x, filter_matrix):
        ...
        if not self.raw_flag:
            ...
            x = x.permute(0,3,1,2).repeat(1, self.dim, 1, 1)  # 5 28 256 256
        else:
            h, w = x.shape
            if filter_matrix.dim() == 5:
                b, c, s, h, w = filter_matrix.size()
            else:
                b, c, s = 1, self.dim, 1
            x = x.unsqueeze(0).unsqueeze(0).repeat(1, self.dim, 1, 1)
        
        x = self.resnet(x)  # 5 28 256 256
            
        mask = filter_matrix.view(b, c*s, h, w).contiguous()
        # mask = filter_matrix[:, 0, :, :, :]
        # Embedding
        ...
        # out = self.mapping(fea)   # 5 28 256 256
        res = out
        # out = self.resnet(res) + res
        return out

if __name__ == '__main__':
    model = MST_with_resnet(dim=4, stage=2, num_blocks=[2, 2], need_noise=True, num_sp=5, need_mask_atten=True, raw_flag=False).cuda()
    dummy_input = torch.randn(2,  1, 4, 100, 100).cuda()
    dummy_filter = torch.randn(2, 1, 4, 100, 100).cuda()
