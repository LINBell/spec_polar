...
class MST(nn.Module):
    ...
    def forward(self, x, filter_matrix):
        ...
        if self.need_mask_atten:
            ...
            for (MSAB, FeaDownSample, _) in self.encoder_layers:
                fea = MSAB(fea, mask)   # 5 28 256 256
                fea_encoder.append(fea)
                fea = FeaDownSample(fea)
...
if __name__ == '__main__':
    model = MST(dim=4, stage=2, num_blocks=[2, 2], need_noise=True, num_sp=5, need_mask_atten=True, raw_flag=False).cuda()
