...
class MST_polo_six(nn.Module):
    def __init__(self, dim=21*6, stage=3, num_blocks=[2,2,2], need_noise=False, num_sp=5, need_mask_atten=False, raw_flag=False):
        super(MST_polo_six, self).__init__()
        self.dim = dim
        self.stage = stage
        self.need_noise = need_noise
        self.num_sp = num_sp
        ...
        # Input projection
        self.embedding = nn.Conv2d(21*6, self.dim, 3, 1, 1, bias=False)
        ...
        # Output projection
        self.mapping = nn.Conv2d(self.dim, 21*6, 3, 1, 1, bias=False)
    ...
    def forward(self, x, filter_matrix):
        ...
        if not self.raw_flag:
            ...
            x = x.permute(0,3,1,2).repeat_interleave(21*6, dim=1)
        else:
            h, w = x.shape
            b, c, s = 1, 21, 6
            x = x.unsqueeze(0).unsqueeze(0).repeat_interleave(21*6, dim=1)
