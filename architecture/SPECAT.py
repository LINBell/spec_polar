...
class SPECAT(nn.Module):
    def __init__(self, dim=28, stage=1, num_blocks=[2,1], need_noise=False, attention_type = 'full', raw_flag=False):
        ...
        self.embedding2 = nn.Conv2d(self.dim, self.dim, 3, 1, 1, bias=False)
    ...
    def forward(self, x, mask=None):
        ...
        if not self.raw_flag:
            x = self.compress(x, mask)
            x = x.permute(0,3,1,2).repeat_interleave(self.dim, dim=1)
        else:

            x = x.unsqueeze(0).unsqueeze(0).repeat_interleave(self.dim, dim=1)


        if mask == None:
            mask = self.lrelu(self.embedding2(x))
        b, c, s, h, w = mask.size()
        mask = mask.view(b, c*s, h, w).contiguous()
...
if __name__ == '__main__':
    model = SPECAT(dim=21, stage=1, num_blocks=[2,1],attention_type = 'full')
    size = 128
    x = torch.randn(1, 21, 1, size, size)
    mask = torch.randn(1, 21, 1, size, size)
    out = model(x, mask)
    print(out.shape)