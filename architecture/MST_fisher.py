...
class Capture_Data(nn.Module):
    def __init__(self, num_data_specdim, curve, nl_in, num_sp, need_noise=True):
        super().__init__()
        self.need_noise = need_noise
        self.nl_in = nl_in
        self.num_sp = num_sp
        self.curve = nn.ModuleList()
        for i in range(self.num_sp * self.num_sp):
            self.curve.append(nn.Conv2d(in_channels=num_data_specdim, out_channels=1, kernel_size=1, stride=1, padding=0, bias=False))
        if curve is None:
            for i in range(self.num_sp * self.num_sp):
                self.curve[i].weight = torch.nn.Parameter(self.curve[i].weight + 0.5)
                print('curve is none')
        else:
            for i in range(self.num_sp*self.num_sp):
                self.curve[i].weight = torch.nn.Parameter(torch.Tensor(curve[i]).reshape(1, num_data_specdim, 1, 1))
            for param in self.curve.parameters():
                param.requires_grad = False

    def forward(self, spec_data, need_noise=False):
        bs, c, h, w = spec_data.size()
        spec_superpixel = []
        cap_superpixel = []


        copy_input = spec_data
        copy_input = copy_input.permute(0, 2, 3, 1).cpu().numpy()

        for index_x in range(self.num_sp):
            for index_y in range(self.num_sp):
                spec_superpixel.append(spec_data[:, :, index_x:h:self.num_sp, index_y:w:self.num_sp])   #list 9 x 5  28  h/3 w/3



        # for i in range(self.num_sp * self.num_sp):
        #     for index in range(c):
        #         spec_superpixel[i][:, index, :, :] *= torch.squeeze(self.curve[i].weight)[index]   # 5 100 100    old version
        # cap_superpixel.append(spec_superpixel[i])  # 9 x 5 28 100 100   need 5 28 300 300
        # cap_data = F.relu(torch.tensor(process_data(cap_superpixel, self.num_sp), dtype=torch.float32).cuda())    # 5 28 300 300


        for index in range(self.num_sp * self.num_sp):
            cap_superpixel.append(F.relu(self.curve[index](spec_superpixel[index])))

        cap_data = process_data(cap_superpixel, self.num_sp)   # 5 1 240 240
        ...
        if self.need_noise:
            ...
            return cap_data, cap_data_noise, lambda_s, lambda_d, sigma_r2
        else:
...
# model =  Capture_Data(28, None, 0.5, 3).cuda()
# data = torch.rand([5, 28, 300, 300]).cuda()
# output = model(data)
# a=1
...
class MST_fisher(nn.Module):
    def __init__(self, dim=21, stage=3, num_blocks=[2,2,2], need_noise=False, num_sp=5):
        super(MST_fisher, self).__init__()
        self.dim = dim
        self.stage = stage
        self.need_noise = need_noise
        self.num_sp = num_sp
        ...
        # Input projection
        self.embedding = nn.Conv2d(21, self.dim, 3, 1, 1, bias=False)

        self.compress = Capture_Data(num_data_specdim=self.dim, curve=None, nl_in=0.5, num_sp=self.num_sp, need_noise=self.need_noise)
        ...
        # Output projection
        self.mapping = nn.Conv2d(self.dim, 21, 3, 1, 1, bias=False)
    ...
    def forward(self, x, mask=None):
        ...
        if self.need_noise:
            cap_data, x, lambda_s, lambda_d, sigma_r2 = self.compress(x, need_noise=self.need_noise)
        else:
            x = self.compress(x, need_noise=self.need_noise)
        x = x.permute(0,3,1,2).repeat_interleave(21, dim=1)
        ...
        return out, cap_data, lambda_s, lambda_d, sigma_r2
