...
class MST_polo(nn.Module):
    def __init__(self, dim=21*4, stage=2, num_blocks=[2,2,2], need_mask_atten=False):
        super(MST_polo, self).__init__()
        self.dim = dim
        self.stage = stage
        self.need_mask_atten = need_mask_atten

        # Input projection
        self.embedding = nn.Conv2d(21*4+1, self.dim, 3, 1, 1, bias=False)
        ...
        # Output projection
        self.mapping = nn.Conv2d(self.dim, 21*4, 3, 1, 1, bias=False)
    ...
    def forward(self, x, filter_matrix):
        ...
        # x = x.permute(0,3,1,2).repeat_interleave(21*4, dim=1)  # 原本输入 [bs, 84, 100, 100]
        ...
        out = self.mapping(fea) + original_embed   # 5 28 256 256
...
class HyPaNet(nn.Module):
    def __init__(self, in_nc=84, out_nc=8, channel=256):
...
class DAUHST(nn.Module):
    ...
    def __init__(self, num_iterations=1):
        ...
        self.para_estimator = HyPaNet(in_nc=84, out_nc=num_iterations*2)
        self.fution = nn.Conv2d(168, 84, 1, padding=0, bias=True)
    ...
    def initial(self, y, Phi):
        """
        :param y: [b,256,310]  ---> [b,200,200]
        :param Phi: [b,28,256,310]  --->[b,4,21,200,200] --->[b,84,200,200]
        :return: temp: [b,28,256,310]; alpha: [b, num_iterations]; beta: [b, num_iterations]
        """
        nC = 84
        y = y / nC * 2
        y_shift = y.unsqueeze(1).repeat_interleave(21*4, dim=1)
        z = self.fution(torch.cat([y_shift, Phi], dim=1))  # [b,84,200,200]
    ...
    def forward(self, y, input_mask=None):
        ...
        :param y: [b,256,310]    ---> [b,200,200]
        :param Phi: [b,28,256,310]  --->[b,4,21,200,200] --->[b,84,200,200]
        :param Phi_PhiT: [b,256,310]  --->[b,200,200]
        :return: z_crop: [b,28,256,256]
        """
        Phi, Phi_s = input_mask
        bs, c, s, h, w = Phi.size()
        denoise_filter_matrxi = Phi
        Phi = Phi.reshape([bs,84,100,100])
        z, alphas, betas = self.initial(y, Phi)
        for i in range(self.num_iterations):
            alpha, beta = alphas[:,i,:,:], betas[:,i:i+1,:,:]
            Phi_z = A(z, Phi)  # [5,200,200]
            x = z + At(torch.div(y-Phi_z,alpha+Phi_s), Phi) #[5,84,200,200]
            # x = shift_back_3d(x)
            beta_repeat = beta.repeat(1,1,x.shape[2], x.shape[3])
            z = self.denoisers[i](torch.cat([x, beta_repeat],dim=1), denoise_filter_matrxi)  # 输入denoise[5,85,100,100]  输出[5,84,100,100]
            # if i<self.num_iterations-1:
            #     z = shift_3d(z)
        return z

if __name__ == '__main__':
    model = DAUHST(num_iterations=3).cuda()
    Phi = torch.rand([5,21,4,100,100]).cuda()  # 测量矩阵
    Phi_s = torch.rand([5,100,100]).cuda()   # 测量矩阵平方加和
    input_mask = [Phi, Phi_s]
    y = torch.rand([5,100,100]).cuda()  # 压缩值
    out = model(y, input_mask)