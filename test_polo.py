...
filter_matrix_test = torch.from_numpy(np.load(filter_path)).unsqueeze(0).repeat(batch_size, 1, 1, 2, 2).cuda().float()
...
for i in range(1, 15):
    ...
    rgb_gt, _ = trans_multi_2_rgb(real_gt[index, :, 0, :, :].transpose(1, 2, 0))
    rgb_out, _ = trans_multi_2_rgb(model_out[index, :, 0, :, :].transpose(1, 2, 0))
    plt.subplot(1, 2, 1)
    plt.imshow(rgb_gt)
    plt.title('gt_rgb')
    plt.subplot(1, 2, 2)
