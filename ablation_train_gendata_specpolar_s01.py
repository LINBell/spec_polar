...
model = unet(CHANNEL=21*2, channel_nums=64, kernel_size=5, raw_flag=False).cuda()
...
def train(epoch, logger):
    ...
    for i in tqdm(range(batch_num), desc=f'Epoch {epoch}', unit='batch'):
        ...
        noisy_filter = add_gaussian_noise(filter_matrix_shuffle, 0.15)[:, :, :2, :, :]
        gt_batch = shuffle_crop_npy_spec_polo(train_set, opt.batch_size, opt.crop_size)  # 9 4 4 200 200
        gt_batch = gt_batch[:, :, :2, :, :]  # 只用前两个极化方向的数据进行训练
        ...
        model_out = model_out.view(opt.batch_size, 21, 2, opt.crop_size, opt.crop_size)
...
def test(epoch, logger):
    ...
    noisy_filter_test = filter_matrix_test[:, :, :2, :, :]
    ...
    test_gt_all = test_gt_all[:, :, :2, :, :]  # 只用前两个极化方向的数据进行测试  
    ...
    noisy_filter_test = noisy_filter_test[:, :, :2, :H, :W]
    ...
    with torch.no_grad():
        for k in range(N):
            ...
            out_tensor = out_tensor.view(1, 21, 2, gt_np.shape[-2], gt_np.shape[-1])
            ...
            # 保存预测和真值
            pred_list.append(np.tile(out_np_hw, (1, 1, 1, 2)))  # H x W x 21 x 4
            truth_list.append(np.tile(gt_np_hw, (1, 1, 1, 2)))  # H x W x 21 x 4
