...
train_set = LoadTraining(train_path, opt.select_num)
test_data = LoadTest(test_path).permute(0, 3, 4, 1, 2)
...
# model = unet(CHANNEL=21, channel_nums=64,/kernel_size=5, raw_flag=False).cuda()
model = SPECAT(dim=21, stage=1, num_blocks=[2,1],attention_type = 'full', need_noise=True).cuda()
model = MST_with_resnet(dim=21, stage=2, num_blocks=[2, 2, 2], need_noise=False, num_sp=5, need_mask_atten=False).cuda()
...
filter_matrix = filter_matrix[:, :, 0, :, :].unsqueeze(2)
filter_matrix_test = filter_matrix_test[:, :, 0, :, :].unsqueeze(2)
...
def train(epoch, logger):
    ...
    for i in tqdm(range(batch_num), desc=f'Epoch {epoch}', unit='batch'):
        ...
        # print(f'Input shape: {input.shape}, Filter shape: {noisy_filter.shape}')
        model_out = model(input, noisy_filter)  # 9 16 200 200
        model_out = model_out.view(opt.batch_size, 21, opt.crop_size, opt.crop_size)
...
def test(epoch, logger):
    ...
    with torch.no_grad():
        for k in range(N):
            ...
            output_full = model(gt_tensor, noisy_filter_test)
