...
model = SPECAT(dim=4, stage=1, num_blocks=[2,1],attention_type = 'full').cuda()
...
def test(epoch, logger):
    ...
    # 定义分块参数
    patch_size = 128  # 网络输入大小
    grid_h = H // patch_size  # 4
    grid_w = W // patch_size  # 4
    ...
    with torch.no_grad():
        for k in range(N):
            ...
            # 创建输出张量用于拼接
            B, C = gt_tensor.shape[0], 4
            output_full = torch.zeros((B, C, H, W), dtype=gt_tensor.dtype, device=gt_tensor.device)
            ...
            for i in range(grid_h):
                for j in range(grid_w):
                    ...
                    # 提取块
                    patch_gt = gt_tensor[:, :, :, h_start:h_end, w_start:w_end]
                    patch_filter = noisy_filter_test[:, :, :, h_start:h_end, w_start:w_end]
                    
                    # 网络推理
                    # print(f"Processing patch: ({i}, {j}), shape: {patch_gt.shape}, mask shape: {patch_filter.shape}")
                    
                    patch_out = model(patch_gt, patch_filter)
                    patch_out = patch_out.view(B, C, patch_size, patch_size)
                    
                    # 将输出放回对应位置
                    output_full[:, :, h_start:h_end, w_start:w_end] = patch_out
