...
train_path = '/data8T/hzx/dataset/HSI/CAVE_1024_28'
test_path = '/data8T/hzx/dataset/HSI/TSA_simu_data/Truth'
train_set = LoadTraining_gen_specpolar(train_path, opt.select_num)
test_data = LoadTest_gen_specpolar(test_path).permute(0, 3, 4, 1, 2)
print(train_set[0].shape)  # 21 4 1024 1024
print(test_data.shape)  # N 21 4 512 612

# saving path
date_time = str(datetime.datetime.now())
date_time = time2file_name(date_time)
result_path = 'ablation_test_split_channel'+ '/' + opt.save_name + '/result/'  # train_t_mask 为1_1
model_path = 'ablation_test_split_channel' + '/' + opt.save_name + '/model/'
...
model = MST(dim=4, stage=2, num_blocks=[2, 2, 2], need_noise=False, num_sp=5, need_mask_atten=False).cuda()
# model = MST_with_resnet(dim=4, stage=2, num_blocks=[2, 2, 2], need_noise=False, num_sp=5, need_mask_atten=False).cuda()
# model = unet(CHANNEL=4, channel_nums=64, kernel_size=5, raw_flag=False).cuda()
# model = SPECAT(dim=4, stage=1, num_blocks=[2,1],attention_type = 'full', need_noise=True).cuda()
...
# filter_matrix = filter_matrix[:, opt.channel_index, :, :, :].unsqueeze(1)
# filter_matrix_test = filter_matrix_test[:, opt.channel_index, :, :, :].unsqueeze(1)
...
def train(epoch, logger):
    ...
    for i in tqdm(range(batch_num), desc=f'Epoch {epoch}', unit='batch'):
        ...
        gt_batch = gt_batch[:, opt.channel_index, :, :, :]
        # spec_data = gt_batch.clone()
        ...
        model_out_01 = (model_out + 1) / 2.0
        real_gt_01 = (real_gt + 1) / 2.0

        # data_range应该是1.0，因为现在范围是[0,1]
        ssim_val = 1 - ssim(model_out_01, real_gt_01, 
                        data_range=1.0,
                        size_average=True,
                        nonnegative_ssim=True)
        # loss = pre_loss_norm 
        loss = pre_loss + 0.4 * ssim_val
    ...
    for k in range(model_out.shape[0]):
        ...
        psnr_train= torch_psnr(model_out[k, 0, :, :], real_gt[k, 0, :, :])
        ssim_train = torch_ssim(model_out[k, :, :, :], real_gt[k, :, :, :])
...
def test(epoch, logger):
    ...
    # 准备测试数据
    test_gt_all = np.stack(test_data, axis=0)
    test_gt_all = np.transpose(test_gt_all, (0, 3, 4, 1, 2))  # N 21 4 H W

    # test_gt_all = test_gt_all[:, :, :, :512, :512]  # 10 21 4 512 512
    ...
    with torch.no_grad():
        for k in range(N):
            gt_np = test_gt_all[k]  # 形状: 21, 4, 512, 512
            gt_tensor = torch.tensor(gt_np[np.newaxis, ...]).cuda().float()
            real_gt = gt_tensor[:, opt.channel_index, :, :, :] 
            
            output_full = model(gt_tensor, noisy_filter_test)  # 形状: 1, 16, H, W
            


            
            out_tensor = output_full
            # 后续处理保持不变
            out_np = out_tensor[0].cpu().numpy()  # 形状: 4, H, W

            # PSNR / SSIM
            psnr_val = torch_psnr(out_tensor[0, 0, :, :], real_gt[0, 0, :, :])
            ssim_val = torch_ssim(out_tensor[0, :, :, :], real_gt[0, :, :, :])
            psnr_list.append(psnr_val.detach().cpu().numpy())
            ssim_list.append(ssim_val.detach().cpu().numpy())

            label = real_gt[0].cpu().numpy()
            # DoP / CPF
            # 重新组织维度顺序为: H, W, S, 4
            out_np_hw = np.transpose(out_np, (1, 2, 0))  # H, W, 4
            gt_np_hw = np.transpose(label, (1, 2, 0))    # H, W, 4
            ...
            # 保存预测和真值
            pred_list.append(out_np_hw)  # 512 x 512 x 4
            truth_list.append(gt_np_hw)  # 512 x 512 x 4
    ...
    model.train()
    print(f"Prediction shape: {pred_list[0].shape}")
    print(f"Truth shape: {truth_list[0].shape}")
...
def main():
    ...
    for epoch in range(1, opt.max_epoch + 1):
        ...
        if (psnr_mean > psnr_max and psnr_mean != np.inf):
            ...
            draw_test_images_only_polar(truth, pred, name, psnr_all, ssim_all)
