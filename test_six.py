...
if opt.method=='hdnet':
    ...
    model = model_generator('spec_polo_six', opt.pretrained_model_path).cuda()

model_path = '9_9/six/model/model_epoch_133.pth'
...
filter_matrix_test = torch.from_numpy(np.load(filter_path)).unsqueeze(0).repeat(15, 1, 1, 2, 2).cuda().float()
...
def stokes_filter_to_hvadlr_torch(filter_stokes):
    ...
    # 堆叠成 [bs, c, 6, h, w]
    filter_hvadlr = torch.stack([H, V, A, D, R, L], dim=2)
    
    return filter_hvadlr



filter_matrix = stokes_filter_to_hvadlr_torch(filter_matrix)
...
test_data = random_load_spec_polo_six(opt.test_path, norm=True)
...
for i in range(1, 6):
    for j in range(1, 6):
        ...
        test_input = torch.tensor(test_gt).cuda().float()
        ...
        with torch.no_grad():
            ...
            model_out = model_out.view(batch_size, 21, 6, 100, 100)
...
for i in range(1, 15):
    ...
    gt_disp = real_gt[index, :, :, :, :]

    out_disp = model_out[index, :, :, :, :]

    psnr= np_psnr(out_disp, gt_disp)
    print(f'psnr:{np_psnr(out_disp, gt_disp)}')
    save_path = rf'temp_val\norm_cap\{index}_{psnr:.2f}'
    if not os.path.exists(save_path):
        os.makedirs(save_path, exist_ok=True)

    gt_disp = stack_21(((real_gt[index, :, 0, :, :]+real_gt[index, :, 1, :, :])/2).transpose(1, 2, 0))
    out_disp = stack_21(((model_out[index, :, 0, :, :]+model_out[index, :, 1, :, :])/2).transpose(1, 2, 0))
    ...
    _, rgb_gt = trans_multi_2_rgb(((real_gt[index, :, 0, :, :]+real_gt[index, :, 1, :, :])/2).transpose(1, 2, 0))
    _, rgb_out = trans_multi_2_rgb(((model_out[index, :, 0, :, :]+model_out[index, :, 1, :, :])/2).transpose(1, 2, 0))
    plt.subplot(1, 2, 1)
    plt.imshow(rgb_gt)
    plt.title('gt_rgb')
    plt.subplot(1, 2, 2)
    plt.imshow(rgb_out)
    plt.title('out_rgb')
    plt.savefig(os.path.join(save_path, '20.png'))
    plt.show()
    ...
    c, h, w = gt_disp.shape
    select_h = random.randint(0, h)
    select_w = random.randint(0, w)
