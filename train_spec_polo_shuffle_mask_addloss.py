...
def weighted_stokes_loss(pred, target, weights=[1.0, 20.0, 20.0, 20.0]):
    """
    分量加权的斯托克斯损失
    pred, target: [bs, 21, 4, h, w]
    weights: 对应S0, S1, S2, S3的权重
    """
    # 计算每个斯托克斯分量的L1损失 [bs, 21, 4, h, w] -> [4]
    loss_per_component = torch.abs(pred - target).mean(dim=[0, 1, 3, 4])
    
    # 应用权重
    weights_tensor = torch.tensor(weights, dtype=torch.float32, device=pred.device)
    weighted_loss = loss_per_component * weights_tensor
    
    s0_loss = weighted_loss[0]
    s123_loss = (weighted_loss[1] + weighted_loss[2] + weighted_loss[3]) / 3.0

    return weighted_loss.mean(), s0_loss, s123_loss
...
result_path = '9_9'+ '/' + opt.save_name + '/result/'  # train_t_mask 为1_1
model_path = '9_9' + '/' + opt.save_name + '/model/'
...
# pre_model_path = '/data8T/lzj/MST_Spectral_remote/simulation/train_code/9_9/basic/model/model_epoch_95.pth'
...
# filter_matrix_test = torch.from_numpy(np.load(filter_path)).unsqueeze(0).repeat(10, 1, 1, 3, 3).cuda().float()
...
def train(epoch, logger):
    ...
    for i in tqdm(range(batch_num), desc=f'Epoch {epoch}', unit='batch'):
        ...
        # 使用分量加权损失
        loss, s0_loss, s123_loss = weighted_stokes_loss(model_out, real_gt, weights=[1.0, 5.0, 5.0, 5.0])
    ...
    logger.info('===> train s0_loss = {:.3f}, s123_loss = {:.3f}'
                .format(s0_loss.detach().cpu().numpy(), s123_loss.detach().cpu().numpy()))
