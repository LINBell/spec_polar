...
train_set = random_load_spec_polo_six(opt.data_path, opt.select_num, norm=False)
# test_data = LoadTraining_npy_spec_polo(opt.test_path, norm=True)
test_data = random_load_spec_polo_six(opt.test_path, norm=False)
...
def train(epoch, logger):
    ...
    for i in tqdm(range(batch_num), desc=f'Epoch {epoch}', unit='batch'):
        ...
        model_out = model_out.view(opt.batch_size, 21, 6, opt.crop_size, opt.crop_size)
        ...
        # loss = pre_loss_norm + 1e-1 * spec_loss 
    ...
    for k in range(model_out.shape[0]):
        ...
        psnr_train= torch_psnr(model_out[k, :, 0, :, :] + model_out[k, :, 1, :, :], input[k, :, 0, :, :] + input[k, :, 1, :, :])
        ssim_train = torch_ssim(model_out[k, :, 0, :, :] + model_out[k, :, 1, :, :], input[k, :, 0, :, :] + input[k, :, 1, :, :])
...
def test(epoch, logger):
    ...
    with torch.no_grad():
        ...
        model_out = model_out.view(10, 21, 6, opt.crop_size, opt.crop_size)

    end = time.time()
    for k in range(test_gt.shape[0]):
        psnr_val = torch_psnr(model_out[k, :, 0, :, :] + model_out[k, :, 1, :, :], test_gt[k, :, 0, :, :] + test_gt[k, :, 1, :, :])
        ssim_val = torch_ssim(model_out[k, :, 0, :, :] + model_out[k, :, 1, :, :], test_gt[k, :, 0, :, :] + test_gt[k, :, 1, :, :])
