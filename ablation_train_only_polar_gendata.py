...
train_set = random_load_spec_polo(opt.data_path, opt.select_num, Loading_dim='Polar')
test_data = LoadTraining_npy_spec_polo(opt.test_path, Loading_dim='Polar')
...
def test(epoch, logger):
    ...
    test_gt_all = test_gt_all[5:, :, :, :512, :512]  # 10 21 4 512 512
    ...
    with torch.no_grad():
        for k in range(N):
            ...
            gt_tensor = torch.tensor(gt_np[np.newaxis, ...]).float().cuda()
            real_gt = gt_tensor[:, 0, :, :, :] 
