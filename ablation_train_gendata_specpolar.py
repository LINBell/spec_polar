...
model = MST_with_resnet(dim=21*4, stage=2, num_blocks=[2, 2, 2], need_noise=False, num_sp=5, need_mask_atten=False, resnet_layers=10).cuda()
# model = unet(CHANNEL=21*4, channel_nums=64*2,kernel_size=5, raw_flag=False).cuda()
# model = MST(dim=21*4, stage=2, num_blocks=[2, 2, 2], need_noise=False, num_sp=5, need_mask_atten=False).cuda()
...
def train(epoch, logger):
    ...
    for i in tqdm(range(batch_num), desc=f'Epoch {epoch}', unit='batch'):
        ...
        gt_s0, gt_s1, gt_s2, gt_s3 = real_gt[:, :, 0, :, :], real_gt[:, :, 1, :, :], real_gt[:, :, 2, :, :], real_gt[:, :, 3, :, :]
        pred_s0, pred_s1, pred_s2, pred_s3 = model_out[:, :, 0, :, :], model_out[:, :, 1, :, :], model_out[:, :, 2, :, :], model_out[:, :, 3, :, :]

        eps = 1e-8
        dop_pred = torch.sqrt(pred_s1**2 + pred_s2**2 + pred_s3**2) / (pred_s0 + eps)
        dop_gt = torch.sqrt(gt_s1**2 + gt_s2**2 + gt_s3**2) / (gt_s0 + eps)
        dop_pred = torch.clamp(dop_pred, 0, 1)
        dop_gt = torch.clamp(dop_gt, 0, 1)
        dop_loss = mse(dop_pred, dop_gt)

        loss_polar = L1_loss(pred_s1, gt_s1) + L1_loss(pred_s2, gt_s2) + L1_loss(pred_s3, gt_s3)

        # loss = pre_loss_norm + 0.4 * ssim_val + 0.4 * dop_loss
        loss = pre_loss_norm + 0.4 * ssim_val + 10 * loss_polar
        # loss = pre_loss_norm 
