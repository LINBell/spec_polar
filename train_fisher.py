...
result_path = opt.outf + '/fisher_l/' + '/result/'
model_path = opt.outf + '/fisher_l/' + '/model/'
...
block_path = 'fisher_model/model_epoch_184.pth'
...
def train(epoch, logger):
    ...
    for i in tqdm(range(batch_num), desc=f'Epoch {epoch}', unit='batch'):
        ...
        model_out, cap_data, lambda_s, lambda_d, sigma_r2 = model(input, input_mask_train)
        # loss = torch.sqrt(mse(model_out, gt))

        c_p, lambda_s, lambda_d, sigma_r2 = cap_data, lambda_s.permute(0, 3, 1, 2).detach(), lambda_d.permute(0, 3, 1, 2).detach(), sigma_r2.permute(0, 3, 1, 2).detach()

        sigma_p = torch.sqrt(c_p + lambda_s + lambda_d + sigma_r2)
        sigma_p_weight = 1 / (2 * (sigma_p ** 4)) + 1 / (sigma_p ** 2)  # 15 25 60 60

        full_pattern = cal_fisher_full(input, model)  #   8 1 160 160 21

        full_pattern_i = full_pattern.unsqueeze(-1)
        full_pattern_j = full_pattern.unsqueeze(-2)
        full_pattern = full_pattern_i * full_pattern_j  # 8 1 160 160 21 21


        fisher_p = sigma_p_weight.unsqueeze(4).unsqueeze(5) * full_pattern  # 15 25 60 60 21 21

        fisher_all_9_9 = torch.sum(fisher_p, axis=(0, 1, 2, 3))

        # print(torch.linalg.matrix_rank(fisher_all_9_9).item())

        fisher = fisher_all_9_9

        try:
            fisher_inv = torch.linalg.inv(fisher)  # 求逆矩阵  9x9
            # fisher_inv = torch.linalg.pinv(fisher)  # 求伪逆  21x21
            # 提取对角元素（各参数的方差下界）
            CRLB = torch.trace(fisher_inv)
            # print(CRLB.item())

        except np.linalg.LinAlgError:
            print("Fisher 信息矩阵不可逆，无法计算 CRLB。")
        ...
        for name, param in model.compress.named_parameters():
            ...
            ax.plot(x, curve_response.reshape((-1)), color_list[flag_count - 1], lw=1)
        plt.pause(0.05)
...
def test(epoch, logger):
    ...
    begin = time.time()
    with torch.no_grad():
        model_out, _, _, _, _ = model(test_input, input_mask_test)
...
def main():
    ...
    psnr_max = 0
    for epoch in range(185, opt.max_epoch + 1):
        train(epoch, logger)
