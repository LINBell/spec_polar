...
for i in range(15):
        ...
        test_input = test_input.float()  # 确保是float32
        ...
        filter_matrix_test = filter_matrix_test[:, :, :, :h, :w].float()
        ...
        with torch.no_grad():
            ...
            model_out = model_out.view(1, 21, 6, h, w)
        out_tensor = model_out
        
        out_np = out_tensor[0].cpu().numpy()  # 形状: 21, 4, H, W
        gt_tensor = torch.tensor(test_data[i][np.newaxis, ...]).cuda()

        # PSNR / SSIM
        psnr_val = torch_psnr(out_tensor[0, :, 0, :, :], gt_tensor[0, :, 0, :, :])
        ssim_val = torch_ssim(out_tensor[0, :, 0, :, :].float(), gt_tensor[0, :, 0, :, :].float())
...
result_path = 'temp_val'+ '/' + '/result/'  # train_t_mask 为1_1
