...
model = MST(dim=21, stage=2, num_blocks=[4, 7, 5], need_noise=True, num_sp=5, need_mask_atten=False, raw_flag=True).cuda()
model = MST_with_resnet(dim=21, stage=2, num_blocks=[2, 2, 2], need_noise=False, num_sp=5, need_mask_atten=False, raw_flag=True).cuda()

model_path = 'ablation_test/only_spec/model/model_epoch_255.pth'
model_path = 'ablation_test2/mst_with_resnet_spec/model/model_epoch_217.pth'  # 泛绿
model_path = 'ablation_test2/mst_with_resnet_spec_l1/model/model_epoch_114.pth'  #会好一点
...
test_data = np.load('color_raw_new/12.7/second_1.npy')  # 70 100 100
test_data = np.load('color_raw/11.8_small_color.npy')  # 70 100 100
...
filter_matrix_test = filter_matrix_test[:, :, 0, :h, :w]
...
with torch.no_grad():
    ...
    model_out = model_out.view(batch_size, 21, h, w).squeeze(0)
...
rgb_v2 = trans_multi_2_rgb_v2(model_out.transpose(1, 2, 0))
rgbs, rgb = trans_multi_2_rgb(model_out.transpose(1, 2, 0))
