...
model = SPECAT(dim=21*4, stage=1, num_blocks=[2,1],attention_type = 'full', need_noise=True).cuda()
model = MST_with_resnet(dim=21*4, stage=2, num_blocks=[2, 2, 2], need_noise=False, num_sp=5, need_mask_atten=False, resnet_layers=5).cuda()
model = unet(CHANNEL=21*1, channel_nums=64, kernel_size=5, raw_flag=False).cuda()
...
def train(epoch, logger):
    ...
    for i in tqdm(range(batch_num), desc=f'Epoch {epoch}', unit='batch'):
        ...
        noisy_filter = add_gaussian_noise(filter_matrix_shuffle, 0.15)[:, :, 1, :, :]
        noisy_filter = noisy_filter[:,:, np.newaxis, :, :]
        gt_batch = shuffle_crop_npy_spec_polo(train_set, opt.batch_size, opt.crop_size)  # 9 4 4 200 200
        gt_batch = gt_batch[:, :, 1, :, :]  # 只用前两个极化方向的数据进行训练
        gt_batch = gt_batch[:, :, np.newaxis, :, :]  # 9 4 1 200 200
        ...
        model_out = model_out.view(opt.batch_size, 21, 1, opt.crop_size, opt.crop_size)
        ...
        # loss = pre_loss_norm + 0.4 * ssim_val + 0.4 * dop_loss
        loss = pre_loss_norm + 0.4 * ssim_val 
        # loss = pre_loss_norm 
...
def test(epoch, logger):
    ...
    noisy_filter_test = noisy_filter_test[:, :, 1, :, :]
    noisy_filter_test = noisy_filter_test[:, :, np.newaxis, :, :]
    ...
    test_gt_all = test_gt_all[:, :, 1, :, :]  # 只用前两个极化方向的数据进行测试  
    ...
    with torch.no_grad():
        for k in range(N):
            ...
            gt_tensor = torch.tensor(gt_np[np.newaxis, :, np.newaxis, :, :]).cuda()  
            out_tensor = model(gt_tensor, noisy_filter_test)  
            out_tensor = out_tensor.view(1, 21, 1, gt_np.shape[-2], gt_np.shape[-1])
            ...
            gt_np_hw = np.transpose(gt_np[:, np.newaxis, :, :], (2, 3, 0, 1))    # H, W, 21, 4
            ...
            # 保存预测和真值
            pred_list.append(np.tile(out_np_hw, (1, 1, 1, 4)))  # H x W x 21 x 4
            truth_list.append(np.tile(gt_np_hw, (1, 1, 1, 4)))  # H x W x 21 x 4
    ...
    logger.info('Testing Epoch {}: PSNR={:.2f}, SSIM={:.3f}'
                .format(epoch, psnr_mean, ssim_mean))

    model.train()
    return pred_list, truth_list, psnr_list, ssim_list, psnr_mean, ssim_mean
...
def main():
    ...
    for epoch in range(1, opt.max_epoch + 1):
        ...
        if (psnr_mean > psnr_max and psnr_mean != np.inf):
            psnr_max = psnr_mean 
            name = result_path + '/' + 'Test_%d_%.2f_%.3f' % (epoch, psnr_max, ssim_mean)
