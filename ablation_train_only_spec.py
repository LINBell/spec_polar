...
test_data = LoadTraining_npy_spec_polo(opt.test_path, norm=True, Loading_dim='Spec')
...
model = ResUnet(in_channels=1, out_channels=21, f_init=64).cuda()
...
def train(epoch, logger):
    ...
    for k in range(model_out.shape[0]):
        ...
        psnr_train= torch_psnr(model_out[k, :, :, :], input[k, :, :, :])
        ssim_train = torch_ssim(model_out[k, :, :, :], input[k, :, :, :])
...
def test(epoch, logger):
    ...
    # 准备测试数据
    test_gt_all = np.stack(test_data, axis=0).transpose(0, 3, 1, 2)  
    test_gt_all = test_gt_all[5:]  # 10 21 4 512 612

    test_gt_all = test_gt_all[:, :, :512, :512] # window size 8

    N = test_gt_all.shape[0]
    H, W = test_gt_all.shape[-2], test_gt_all.shape[-1]
    noisy_filter_test = noisy_filter_test[:, :, 0, :H, :W]
    ...
    with torch.no_grad():
        for k in range(N):
            ...
            out_tensor = out_tensor.view(1, 21, gt_np.shape[-2], gt_np.shape[-1])
            out_np = out_tensor[0].cpu().numpy()  # 形状: 21, 4, H, W

            # PSNR / SSIM
            psnr_val = torch_psnr(out_tensor[0, :, :, :], gt_tensor[0, :, :, :])
            ssim_val = torch_ssim(out_tensor[0, :, :, :], gt_tensor[0, :, :, :])
            psnr_list.append(psnr_val.detach().cpu().numpy())
            ssim_list.append(ssim_val.detach().cpu().numpy())

            # DoP / CPF
            # 重新组织维度顺序为: H, W, S, 4
            out_np_hw = np.transpose(out_np, (1, 2, 0))  # H, W, 21, 4
            gt_np_hw = np.transpose(gt_np, (1, 2, 0))    # H, W, 21, 4
